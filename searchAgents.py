...
def cornersHeuristic(state, problem):
    ...
    for corner in corners:
        x1, y1 = corner  # get coordinates of each corner
        manDist = abs(x1 - x) + abs(y1 - y)  # measure manhattan distance from current coords to the corner coords
        distance = max(distance, manDist)  # keep track of the max manhattan distance and return it
    return distance
...
def foodHeuristic(state, problem):
    ...
    "*** YOUR CODE HERE ***"
    # gets 5/4 but is a bit slow. Using manhattan distance instead of mazeDistance is faster but expends more nodes (3/4)

    foodList = foodGrid.asList()  # convert foodGrid into a list
    distance = 0  # keeps track of distance from current position to every food node
